  ...
  >
    <CModalHeader>
      <CModalTitle id="LiveDemoExampleLabel"
        >{{ mode === 'update' ? 'Edit' : mode === 'delete' ? 'Delete' : 'Add' }} Kanagata
        Data</CModalTitle
      >
    </CModalHeader>
        ...
        <CCol>
          <CForm>
            ...
            <CFormInput
              type="number"
              id="actualShot"
              floatingLabel="Actual Shot Kanagata"
        ...
        <CCol>
          <CForm>
            <CFormInput
              type="number"
              id="limitShot"
              floatingLabel="Limit Shot Kanagata"
    ...
    <CModalBody v-else>
      Are you sure you want to delete this kanagata with ID : {{ idKanagata }}, Actual Shot :
      {{ actualShot }}, Limit Shot : {{ limitShot }} ?
    </CModalBody>
    <CModalFooter>
    ...
    const updateKanagata = async () => {
      try {
        const response = await axios.patch(
          `http://192.168.148.125:5000/kanagata/${idKanagata.value}`,
          {
            id_kanagata: idKanagata.value,
            actual_shot: actualShot.value,
            limit_shot: limitShot.value,

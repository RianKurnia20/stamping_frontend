...
<script setup>
    ...
    import formatDate from '@/middleware/LocaleDate';
      ...
      watch(() => props.eventTable.refreshMachine, (newValue) => {
        if (newValue) {
          fetchData();
          // eslint-disable-next-line vue/no-mutating-props
          props.eventTable.refreshMachine = false;
          instance.emit('close')
          instance.emit('notif')
        }
      });
      ...
      const cols =
          ref([
              { field: 'id_machine', title: 'ID Machine', isUnique: true },
              { field: 'shift', title: 'Shift Type' },

<template>
    ...
    <div>
        <vue3-datatable :rows="rows" :columns="cols" :loading="loading" :sortable="true" skin="bh-table-hover bh-table-bordered bh-table-compact" :search="search">
            <template #id="data">
                <strong>{{ data.id_pca }}</strong>
            </template>
            <template #actions="data" v-if="userRole !== 'viewer'">
                <div class="flex gap-4">
                    <CButton size="sm" class="btn-primary" style="margin: 0.1rem;" @click="editItem(data.value)" ><CIcon icon="cil-pencil" /></CButton>
...
<script setup>
    ...
    import formatDate from '@/middleware/LocaleDate';
      ...
      watch(() => props.eventTable.refreshPca, (newValue) => {
        if (newValue) {
          fetchData();
          // eslint-disable-next-line vue/no-mutating-props
          props.eventTable.refreshPca = false;
          instance.emit('close')
          instance.emit('notif')
        }
      });
      ...
      const cols =
          ref([
              { field: 'id_pca', title: 'ID PCA', isUnique: true },
              { field: 'id_machine', title: 'ID Machine' },
              { field: 'id_kanagata', title: 'ID Kanagata' },
      ...
      const fetchData = () => {
        axios.get('http://192.168.148.125:5000/pca')
          .then(response => {

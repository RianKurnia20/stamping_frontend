  ...
  >
    <CModalHeader>
      <CModalTitle id="LiveDemoExampleLabel"
        >{{ mode === 'update' ? 'Edit' : mode === 'delete' ? 'Delete' : 'Register' }} PCA
        Data</CModalTitle
      >
    </CModalHeader>
        ...
        <CCol>
          <CForm>
            ...
            <CFormInput
              type="number"
              id="speed"
              floatingLabel="Stamping Per Minutes"
    ...
    <CModalBody v-else>
      <div>
        Are you sure you want to delete this PCA data with ID Machine : {{ idMachine }}, ID Kanagata
        : {{ idKanagata }}, DP Code : {{ idProduct }}, SPM : {{ speed }} ?
      </div>
    </CModalBody>
    <CModalFooter>
  ...
  setup(props) {
    ...
    const fetchOptionsData = async (endpoint, labelProp, valueProp) => {
      try {
        ...
        return [{ label: 'Open this select menu', value: '' }, ...options]
      } catch (error) {
    ...
    onMounted(async () => {
      machineOptions.value = await fetchOptionsData(
        'http://192.168.148.125:5000/machine',
        'id_machine',
        'id_machine',
      )
      kanagataOptions.value = await fetchOptionsData(
        'http://192.168.148.125:5000/kanagata',
        'id_kanagata',
        'id_kanagata',
      )
      productOptions.value = await fetchOptionsData(
        'http://192.168.148.125:5000/product',
        'name',
        'id_product',
      )
    })
